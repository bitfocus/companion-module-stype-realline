...
| `$(stype-realline:active_count)`           | Number of trackers currently recording    |
...
The Record - All Trackers preset is a single oscillating button: it shows a red record circle (●) when idle and switches to a stop square (■) on a red background while recording. Pressing it toggles recording for all trackers.
